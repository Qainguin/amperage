...
async function runBuild(buildId: string): Promise<void | Issue[]> {
	...
	return new Promise(async (resolve, reject) => {
		...
		if (pros) {
			let ez = false;
			try {
				ez = (await readFile(`builds/${buildId}/project.pros`, { encoding: 'utf-8' })).includes(
					'EZ-Template'
				);
			} catch (err: any) {
				ez = false;
...
const ISSUE_REGEX =
	/^(?<filepath>[^:]+):(?<lineNumber>\d+):\d+: (?<type>error|warning): (?<message>.*)/gm;
...
wss.on('connection', function connection(ws) {
	...
	ws.send(
		JSON.stringify({
			message: `Connected to the 'make' compilation server on port ${WS_PORT}. Ready for ZIP files.`
		})
	);
});
