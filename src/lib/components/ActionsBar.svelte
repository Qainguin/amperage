<script lang="ts">
	...
	let buildState: 'not-building' | 'building' | 'success' | 'fail' = $state('not-building');
...
>
	...
	>
		<Blocks
			size={16}
			class="{buildState === 'success' ? 'text-[#4E4EFF]' : 'build'} {buildState === 'fail'
				? 'text-[#FF3333]'
				: 'build'}"
		></Blocks>
	</button>
	...
	{#if device}
		...
		<button
			...
			onclick={async () => {
				...
				if (device)
					await uploadProject(device, currentSlot, false, window.projectName, window.pfs, () => {
			...
			oncontextmenu={async (e) => {
				...
				uploadState = 'uploading';
				if (device)
					await uploadProject(device, currentSlot, true, window.projectName, window.pfs, () => {
						uploadState = 'not-uploading';
					});
			}}
		>
